...
require_once __DIR__ . '/report-storage.php';
...
if (isset($responseData['candidates'][0]['content']['parts'][0]['text'])) {
    ...
    if ($firstBracket !== false && $lastBracket !== false) {
        ...
        if (json_last_error() === JSON_ERROR_NONE && isset($decodedJson['basicReport']['scores'])) {
            // Store the report data and get a unique report ID
            $reportId = storeReportData($website_url, [
                'website_url' => $website_url,
                'customer_name' => $name,
                'customer_email' => $email,
                'report_data' => $decodedJson,
                'created_at' => date('Y-m-d H:i:s')
            ]);
            
            // Add the report ID to the response
            $decodedJson['reportId'] = $reportId;
            
            echo json_encode($decodedJson); // Success! Send the JSON with report ID to the frontend.
        } else {
